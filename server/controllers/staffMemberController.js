...
async function locationHelper(officeLocation) {
  ...
  if (!refLocation) return { error: 'Sorry room not found' };
    ...
    if (occupied.length >= refLocation.capacity) return { error: 'Sorry room capacity is full' };
...
async function updateInfoHelper(user) {
  ...
  if (!newStaff) return { error: 'No staff with this id' };
  else {
    if (gender) {
...
exports.registerStaff = async function (req, res) {
  try {
    ...
    if (req.body.type === 'Academic Member') JOI_Result = await validation.registerACSchema.validateAsync(req.body);

    let { name, gender, email, salary, officeLocation, type, role, dayOff, faculty, department } = req.body;
...
exports.updateStaff = async function (req, res) {
  try {
    ...
    if (!newStaff) return res.send({ error: 'No staff with this id' });
    ...
    if (req.body.department && !req.body.faculty && newStaff.type === 'Academic Member') return res.send({ error: 'Sorry Please the new faculty is required' });
    if (req.body.faculty && !req.body.department && newStaff.type === 'Academic Member') return res.send({ error: 'Sorry Please the new department is required' });
...
exports.updateStaffDayOff = async function (req, res) {
  try {
    ...
    const newStaff = await StaffMember.findOne({ gucId: req.body.gucId });
    if (!newStaff) return res.send({ error: 'No staff with this id' });
    if (newStaff.type === 'HR') return res.send({ error: 'Sorry days off for HR is Saturday' });
...
exports.deleteStaff = async function (req, res) {
  try {
    ...
    if (!staff) return res.send({ error: 'No staff with this ID' });
    ...
    allCourses.forEach(async (course) => {
      ...
      if (length == 0) course.coverage = 0;
      else course.coverage = (temp / length) * 100;
...
exports.signIn = async function (req, res) {
  try {
    ...
    if (!staff) return res.send({ error: 'Staff not registered in the system' });
    else {
      const currentTime = new Date();
      if (currentTime.getDay() === 5) return res.send({ error: 'Sorry you cannot sign in on Friday' });

      let days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];

      let time = currentTime.getHours() + ':' + currentTime.getMinutes() + ':' + currentTime.getSeconds();

      if (currentTime.getHours() < 7) time = '7:00:00';
      if (currentTime.getHours() > 19) time = '19:00:00';
...
exports.signOut = async function (req, res) {
  try {
    ...
    if (!staff) return res.send({ error: 'Staff not registered in the system' });
      ...
      if (today.getHours() > 19) currentTime = '19:00:00';
...
exports.changePassword = async function (req, res) {
  try {
    ...
    if (!newPassword) return res.send({ error: 'Please enter the new password' });
    if (!oldPassword) return res.send({ error: 'Please enter the old password' });

    const userToEdit = await StaffMember.findOne({ gucId: user.gucId });
    if (!userToEdit) return res.send({ err: 'No user' });
...
exports.getProfile = async function (req, res) {
  try {
    ...
    if (!staff) return res.send({ err: 'No user' });
    ...
    if (!staff.officeLocation) newStaff.officeLocation = 'N/A';
