    ...
    useEffect(async()=>{
		...
		if (!loggedInUser) {
		  document.location.href = '/login'  
            ...
            try{
                const response = await axios.get(`${link}/staffMembers/viewMySchedule`);
                if(response.data.error){
                    ...
                    for(var i =0;i<slotsDisplay.length;i++){
                        switch(slotsDisplay[i].day){
                            case 'Saturday': sat.push(slotsDisplay[i]);break;
                            case 'Sunday': sun.push(slotsDisplay[i]);break;
                            case 'Monday': mon.push(slotsDisplay[i]);break;
                            case 'Tuesday': tues.push(slotsDisplay[i]);break;
                            case 'Wednesday': wed.push(slotsDisplay[i]);break;
                            case 'Thursday': thurs.push(slotsDisplay[i]);break;
                        }
                    }
                ...
                <Table class = "table-slots-style border" aria-label="customized table" size="small">
                    ...
                    <TableBody>
                        <TableRow key = "Saturday">
                            ...
                            {sats.map((day) => (
                            <TableCell align = "center" className = {typeof(day) === 'string'?"border":"border occupiedS"} >{typeof(day) === 'string'?day:day.Location} <br/>&nbsp;{typeof(day) === 'string'?"\t":day.Course}</TableCell>
                            ))}
                        </TableRow>
                        <TableRow key = "Sunday">
                            <TableCell component="th" scope="row" className="yellow">
