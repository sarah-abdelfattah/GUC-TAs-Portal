  ...
  const {addToast} = useToasts(); 
    ...
    useEffect(async()=>{
        try{
            ...
            const response =   await axiosCall("get", "requests/viewRecievedReplacementRequest");
      ...
      let myRequests  = response.data.data.map((req) => {
             ...
             var y= new Date(Date.parse(req.replacemntDate)).getDate()+"/"+(new Date(Date.parse(req.replacemntDate)).getMonth()+1)+"/"+new Date(Date.parse(req.replacemntDate)).getFullYear()+" at "+new Date(Date.parse(req.replacemntDate)).getHours()+":"+new Date(Date.parse(req.replacemntDate)).getMinutes()
              return {
                id:req._id, 
                 date:x,
     ...
     const handleSubmit = async (e,rowData) =>{
        ...
        try{
           let submittedStatus = e.currentTarget.value;
            ...
            const response =   await axiosCall("put", 'requests/AcceptOrRejectRep/'+rowData.id , { AcceptOrReject: submittedStatus});
                ...
                if(submittedStatus === "accepted" && response.data.data === 'you cannot accept this request, you do not have this free slot in your Schedule'){
                    addToast('you cannot accept this request, you do not have this free slot in your Schedule',
...
else
{
  ...
  const classes = useStyles();
    return (
        ...
        <Fade>
        <h3 className="general-header">Replacement Requests</h3>
